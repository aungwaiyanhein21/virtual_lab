...
def db_get_level_problems(con, id):
    sql = "SELECT * FROM BundleProblem WHERE bundle_id = :id"
    params = {'id': id}
    rows, error = db_query(con, sql, params)
    if rows is None:
        return None, error
    ...
    if len(rows) == 0:
        error = "LevelProblem id {} not found.".format(id)
        return None, error
...
def db_get_game_bundles(con, id):
    sql = "SELECT * FROM CourseBundle WHERE course_id = :id"
    params = {'id': id}
    rows, error = db_query(con, sql, params)
    if rows is None:
        return None, error
    ...
    if len(rows) == 0:
        error = "CourseBundle id {} not found.".format(id)
        return None, error
...
def db_delete_game(conn, id):
    ...
    result, error = db_transaction_update(conn, sqls, param_lists)
    if result is None:
        return None, "Failed to delete game: database error"
